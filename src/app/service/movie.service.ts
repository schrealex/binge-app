...
export class MovieService {
    ...
    favoriteMovies(): Observable<Movie[]> {
        return this.apiAuthenticationService.getSessionId().flatMap(sessionId => {
            return this.http.get(favoriteMoviesUrl(sessionId, 1)).flatMap(pagesResult => {
                const calls = [];
                for (let i = 1; i <= pagesResult.json().total_pages; i++) {
                    calls.push(this.http.get(favoriteMoviesUrl(sessionId, i)).map(result => <Movie[]> result.json().results));
                }
